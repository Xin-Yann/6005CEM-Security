...
async function fetchDataAndDisplay() {
    try {
        ...
        const subcollectionRef = collection(catDocRef, productType);
        ...
        documents.forEach(foodData => {
            ...
            ['product_id', 'product_image', 'product_name', 'product_description', 'product_price', 'product_stock', 'product_weight'].forEach(field => {
                ...
                if (field === 'product_image' && foodData[field]) {
                    ...
                    productImage.src = `/image/products/cat/${productType}/${foodData[field]}`;
...
async function deleteProduct(productId, productType) {
    try {
        const productRef = doc(db, `products/cat/${productType}/${productId}`);
        await deleteDoc(productRef);
        alert('Product deleted successfully!');
        
        // Log the deletion action in the staff_action collection
...
function editProduct(productId, productType) {
    window.location.href = `/html/staff/staff-editproduct.html?category=cat&id=${productId}&type=${encodeURIComponent(productType)}`;
}
